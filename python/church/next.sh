	...
	if [[ "yes" = "${SELECTED_YN}" ]]; then
		while [[ "no" != "${SELECTED_YN}" ]]; do
			if [[ "yes" = "${SELECTED_YN}" ]]; then
				...
				fmt.pl -f "${WORK_DIR}/stackoverflow_${COUNT}_${ID}.py"
				build "${ID}"
			fi

			echo "# help     = python3 main.py -i ${ID}"
			select_yn "Run ID: ${ID}"
		done
	fi
	# ----------------------------------------------------------------
...
if [[ -n "${INPUT}" ]]; then
	...
	readonly RUST="${WORK_DIR}/stackoverflow_${COUNT}_${ID}.py"
	echo "# RUST     = ${RUST}"
