...
'''
# @filename     :  stackoverflow_0120_1747817.py
...
class Answer1:
    ...
    def code1():
        ...
        pairs = [('a', 1), ('b', 2)]
        dictionary = dict(pairs)  # => {'a': 1, 'b': 2}
        assert {'a': 1, 'b': 2} == dictionary
        dictionary = dict([(k, v + 1) for k, v in pairs])  # => {'a': 2, 'b': 3}
        assert {'a': 2, 'b': 3} == dictionary

        # Given separate arrays of keys and values, use the dict constructor with zip:
        keys = ['a', 'b']
        values = [1, 2]
        dictionary = dict(zip(keys, values))  # => {'a': 1, 'b': 2}
        assert {'a': 1, 'b': 2} == dictionary
